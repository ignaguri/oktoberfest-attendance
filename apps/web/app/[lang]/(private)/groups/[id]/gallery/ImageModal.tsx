...
export function ImageModal({ imageUrl, photoId, groupId, onClose }: ImageModalProps) {
  ...
  // Who is in a photo needs only its id; reactions and comments live in a group
  const hasPanel = !!photoId;
  const hasSocial = hasPanel && !!groupId;
  ...
  return (
    <Dialog open={!!imageUrl} onOpenChange={onClose}>
      ...
      >
        ...
        <div className={cn("flex", hasPanel ? "flex-col sm:flex-row" : "")}>
          {/* Photo */}
          <div
            className={cn(
              "relative",
              hasPanel ? "min-h-[40vh] flex-1 sm:min-h-[70vh]" : "h-[80vh] w-full",
            )}
          >
            <Image
              src={imageUrl}
              ...
              priority
              sizes={hasPanel ? "(max-width: 640px) 100vw, 60vw" : "90vw"}
              unoptimized
            />
          </div>

          {/* Side panel: tags whenever there is a photoId, reactions and comments only with a group */}
          {hasPanel && (
