...
export const GROUP_ACHIEVEMENT_UNLOCKED_WORKFLOW_ID = "group-achievement-unlocked" as const;

export const groupAchievementUnlockedPayloadSchema = z.object({
  groupName: z.string().describe("Name of the group that unlocked the achievement"),
  ...
  contributorName: z.string().describe("Name of the member who contributed to unlocking"),
  unlockedAt: z.string().describe("When the achievement was unlocked"),
});

export type GroupAchievementUnlockedPayload = z.infer<typeof groupAchievementUnlockedPayloadSchema>;

export const groupAchievementUnlockedWorkflow = workflow(
  GROUP_ACHIEVEMENT_UNLOCKED_WORKFLOW_ID,
  async ({ step, payload }: { step: any; payload: GroupAchievementUnlockedPayload }) => {
