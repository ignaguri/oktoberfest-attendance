-- Make "tents were not supplied" distinguishable from "remove every tent".
--
-- update_personal_attendance_with_tents treated NULL and an empty array
-- identically, deleting every tent visit for the date. That made data loss the
-- default failure mode for any caller that lost track of the array: the mobile
-- attendance form initialised its tent field from a list query that never
-- carries tentVisits, so opening a logged day and pressing Save sent an empty
-- array and silently wiped that day's tent visits. The empty-array branch also
-- reported tents_removed as empty while deleting rows, so callers could not
-- even observe the deletion.
--
-- New contract:
--   p_tent_ids IS NULL   -> leave tent visits untouched (nothing was supplied)
--   p_tent_ids = '{}'    -> the caller explicitly cleared the selection, remove all
...
BEGIN
    ...
    VALUES (p_user_id, p_date::date, p_festival_id)
