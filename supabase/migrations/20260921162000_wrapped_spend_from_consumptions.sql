-- Wrapped reported a spend nobody paid.
--
-- get_wrapped_data priced everything as `beers * festivals.beer_cost`, in five
-- places. That was true when it was written: 2023 and 2024 really were flat
-- rate, and both still reconcile to the cent. It stopped being true once tents
-- carried their own prices and users tipped, so today it ignores the tent, the
-- tip, and every non-beer drink (a soft drink contributes 0 to your spend).
--
-- Measured against prod when this was written: Oktoberfest 2026 understates by
-- 6.35 EUR on average and 17.40 EUR at worst, Oktoberfest 2025 overstates by
-- 6.73 and 21.05. The previous-festival comparison was wrong twice over, since
-- it priced last year's beers at this year's rate.
--
-- Spend now comes from consumptions.price_paid_cents, which is the number the
-- user was actually charged and, since 20260921135451, the one the server
-- resolves rather than the client guessing.
--
-- `beer_cost` stays in the payload: it is the festival's headline price, which
-- is a real thing to show, just not a way to total a bill.

-- The money counterpart of _get_effective_drink_count, and it falls back the
-- same way: an attendance old enough to predate consumption rows has only
-- beer_count, so price that at its OWN festival's rate rather than the rate of
-- whichever festival is being rendered.
CREATE OR REPLACE FUNCTION public._get_effective_spend_cents(p_attendance_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
...
AS $function$
  SELECT COALESCE(
    (SELECT SUM(c.price_paid_cents)::int
       FROM consumptions c
      WHERE c.attendance_id = p_attendance_id),
    (SELECT ROUND(a.beer_count * COALESCE(f.beer_cost, 16.20) * 100)::int
       FROM attendances a
...
ALTER FUNCTION public._get_effective_spend_cents(uuid) OWNER TO postgres;

-- Service role only, matching _get_effective_drink_count. This is SECURITY
-- DEFINER and takes a bare attendance id, so a grant to `authenticated` would
-- let any signed-in user read any other user's spend. get_wrapped_data is
-- itself SECURITY DEFINER and owned by postgres, so its internal call needs no
-- grant.
--
-- anon and authenticated are revoked by name, not just via PUBLIC: Supabase's
-- default privileges on schema public grant EXECUTE to both roles directly at
-- creation time, so revoking PUBLIC alone leaves the function wide open.
REVOKE ALL ON FUNCTION public._get_effective_spend_cents(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._get_effective_spend_cents(uuid) FROM anon;
REVOKE ALL ON FUNCTION public._get_effective_spend_cents(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public._get_effective_spend_cents(uuid) TO service_role;

COMMENT ON FUNCTION public._get_effective_spend_cents(uuid) IS
  'Cents actually paid on an attendance: the sum of its consumptions, falling back to beer_count priced at that attendance''s own festival rate.';
