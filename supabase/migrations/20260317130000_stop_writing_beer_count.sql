-- Stop writing beer_count in RPC functions
-- The attendance_with_totals view already computes counts from consumptions,
-- falling back to beer_count only when no consumptions exist.
-- All historical data has been backfilled with consumptions.
-- We keep the beer_count column and view fallback intact; this migration
-- only stops the RPCs from writing to it.

-- 1. Rewrite add_or_update_attendance_with_tents
-- Keep p_beer_count in the signature for backwards compat, but ignore it.
...
-- 2. Rewrite update_personal_attendance_with_tents
-- Keep p_beer_count in the signature for backwards compat, but ignore it.
