-- Group rank in Wrapped (top_3_rankings) and profile Highlights (top_positions)
-- was computed with ROW_NUMBER() after filtering to the requesting user, so the
-- partition held one row and every member of every group was "#1". Both now take
-- the member's position from get_group_leaderboard, which already orders by the
-- group's own winning criterion (including tents visited and longest streak) with
-- the total_beers tie-break. WITH ORDINALITY numbers rows in the order the
-- function returns them.
--
-- get_wrapped_data: body identical to 20260921162000 except the top_rankings CTE.
-- CREATE OR REPLACE keeps both functions' existing grants and SECURITY settings.
