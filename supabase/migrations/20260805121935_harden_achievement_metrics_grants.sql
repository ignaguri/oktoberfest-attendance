-- Harden get_achievement_metrics against cross-user data disclosure.
--
-- get_achievement_metrics is SECURITY DEFINER, owned by postgres (rolbypassrls =
-- true), so it bypasses RLS on every table it reads. The prior migration left the
-- default PUBLIC EXECUTE grant in place and the body had no ownership check, so an
-- unauthenticated client holding only the public anon key could call
-- rpc('get_achievement_metrics', { p_user_id: <any other user>, ... }) and read that
-- user's spending, tips, friend count, drink totals and profile completeness. Any
-- logged-in user could do the same for anyone else.
--
-- Mirrors the get_wrapped_data fix in 20260726190000_fix_wrapped_rls_and_comparisons.sql:
-- convert to plpgsql, add an ownership guard as the first statement, and revoke the
-- PUBLIC/anon EXECUTE grants. The metric query itself is unchanged, byte-for-byte.
...
DECLARE
  v_result jsonb;
BEGIN
  -- SECURITY DEFINER: this reads across users, so it must not be callable for
  -- someone else's user id. auth.uid() IS NULL means there is no JWT
  -- (service_role, or an internal server-side call), which is allowed; anon is
  -- revoked below instead.
