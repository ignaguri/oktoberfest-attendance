...
-- Create super_admin role if it doesn't exist (used for function grants)
DO $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."add_or_update_attendance_with_tents"("p_user_id" "uuid", "p_date" timestamp with time zone, "p_beer_count" integer, "p_tent_ids" "uuid"[], "p_festival_id" "uuid") RETURNS TABLE("attendance_id" "uuid", "tents_changed" boolean)
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    v_tent_id UUID;
...
CREATE POLICY "Allow authenticated users to upload" ON "storage"."objects" FOR INSERT TO "authenticated" WITH CHECK ((("bucket_id" = 'beer_pictures'::"text") AND ("auth"."role"() = 'authenticated'::"text")));



CREATE POLICY "Allow public read access" ON "storage"."objects" FOR SELECT USING (("bucket_id" = 'beer_pictures'::"text"));
